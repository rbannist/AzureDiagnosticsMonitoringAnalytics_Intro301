...
This lab is focused on the operational aspects of working with Azure with ITIL Service Operation practices in the areas of Problem Management, Event Management, and Incident Management serving as an input in regard to the original scoping of lab subject areas.
...
You created a Virtual Machine in a previous lab so there should be logs related to the creation of a Virtual Machine resource and other supporting resources such as a NIC, a Storage Account, an OS disk, etc. in your Activity Log.  So, let's look at Activity Logs related to that lab.
...
We're specifically interested in the creation of a Virtual Machine resource so expand that log entry.  Here you will see some of the major changes related to the creation of your Virtual Machine resource.

![Activity Log VM entries](images/3_ActivityLogVM.png?raw=true)

<br>

For example, if you expand the 'Write-NetworkInterfaces' entry and select 'JSON' you will be able to see the Public IP that was bound to the NIC of the VM at this time (that's if your VM had a PIP associated at the time of creation).
...
## Exercise 3 - Setting an Alert

The goal of the Alerts experience is to be the single place to view and manage all your Azure alerts.  In this exercise, before visiting the full Alerts Dashboard, we will setup an alert in advance.
...
VM Boot Diagnistics logs and Guest OS-level Diagnostics Logs fall under 'Event Management' from an ITIL viewpoint and visibility of these logs can aid efforts related to maintaining the health of IT services.  VM Boot Diagnistics logs are Resource-level diagnostics logs.
